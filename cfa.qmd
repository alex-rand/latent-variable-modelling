...
When I'm doing factor analysis, my goal is to convince my research peers that the observed variables are providing a way of measuring the unobservable 'factor' I'm purporting to exist. This seems like an ontologically dubious framing, and it also seems impossible to prove. But people who do research have settled on a few ways of trying to make this case.

One such way is to take all of the measured variables I'm imagining to be caused by the same unmeasured factor and show that they are indeed correlated with each other. When this happens, I can say that my factor has **Convergent Validity.** In the words of @gorsuch1983:
...
3.  Are the within-factor loadings pretty similar to each other?
...
So to get some ideas on how we might improve our goodness-of-fit, let's print out the modification indexes for each of the fixed parameters in the model:
...
Do an example from here, or possibly from the linked 3rd lecture that covers more advanced CFA topics https://stats.oarc.ucla.edu/r/seminars/rcfa/
