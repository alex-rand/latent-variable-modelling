...
### Factor Loadings & Convergent Validty
...
h1.summary$pe %>% 
  ...
  mutate(
    across(.cols = c("est", "se"), round, 2),
    est = paste0(est, " (se = ", se, ", pvalue = ", pvalue, ")")
  ) %>% 
  ...
  knitr::kable(caption = "Factor loadings, standard errors, and p-values")

```

Firstly, notice that the loading of the first factor of each variable is automatically fixed at 1. This is just a default convention we adopt to make sure that the other loadings are identifiable -- you can override this default, but you'd better have an alternative plan for making sure things are identifiable.

Remember, my goal is to convince my research peers that the observed variables are actually providing a way of measuring the unobservable 'factor' I'm purporting to exist. One way of making this case is to look for **Convergent Validity.** Essentially what we'd like to see is that all of a factor's variables load really highly on that factor, but also that they all load to more or less the same degree. In the words of @gorsuch1983:
...
Actually, there seems to be another conventional thing people do to test for convergent validity, which neither @gorsuch1983 nor @Finch2015 mention: we can calculate the **Average Variance Extracted (AVE),** which is the average of the within-factor squared factor loadings. This is based on the idea that a squared factor loading is the variance explained of the variable by that factor. The convention is that if the AVE \> 0.5, then you can feel good about claiming convergent validity. I guess this makes sense -- seems like a pretty simple and ad-hoc way of asking whether your loadings are generally on the same page. But obviously if I have lots of observed variables defining the factor then I'm at risk of having a bunch of high loadings and a bunch of low loadings, resulting in a misleadingly moderate average? Seems risky. Let's do it anyway, for posterity.

@Kline2011 mentions this whole AVE thing in reference to the concept of **reliability,** which he talks about as related to convergent validity. This is where

@Kline2011 evens goes as far as to say that the standardized loadings can in isolation be taken as a measure of convergent validity. In his words on page 344:
...
A few other standardized coefficients are
rather low, such as .433 for the self-talk indicator of constructive thinking, so evidence
for convergent validity is mixed

Rather than doing it manually, we can use a ready-made function from the **semTools** package
...
@gorsuch1983 suggests we go a step further and do some additional modelling to assess the degree of discriminant validity here:
...
The thing to notice here is that the observables with the biggest loadings will also have the smallest residual variances, unsurprisingly. Be sure to focus on the `Std.all` column when doing these assessments, because [that's what this person does](https://www.youtube.com/watch?v=XnHFWe7RFr0). This lets you do fair comparisons between the loadings and residual variance parameters, since it constraints them all to the scale of a correlation coefficient, IE \[-1, 1\]. I SHOULD FIGURE OUT HOW THIS ALL PLAYS IN FOR FIGURING OUT CONVERGENT VALIDITY -- I THINK GORSUCH SAYS THE WITHIN-FACTOR LOADINGS SHOULDN'T BE SIGNIFICANTLY DIFFERENT FROM EACH OTHER? FIND THAT QUOTE
