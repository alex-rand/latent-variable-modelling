...
fit.test <- brm(
 ...
 # prior = priors,
  iter = 2000,
  file = "source/fits/brmstest2-delete"
)
...
model {
  ...
  for (n in 1 : N) {
    if (cens[n] == 0) {
      target += weibull_lpdf(Y[n] | shape, mu_weibull[n] / tgamma(1 + 1 / shape));
    } else if (cens[n] == 1) {
      target += weibull_lccdf(Y[n] | shape, mu_weibull[n] / tgamma(1 + 1 / shape));
    } 
  }
}
...
generated quantities {
  ...
  vector[N] y_pred;
  for (n in 1:N) {
    y_pred[n] = weibull_rng(shape, mu_weibull[n] / tgamma(1 + 1 / shape));
  }
}
...
# Save the resulting model fit
saveRDS(fit, "source/fits/b07.04.rds")
...
fit <- readRDS("source/fits/b07.04.rds")

# If you need to check variable names, call this
#test <- fit$summary()
...
# Save the result
saveRDS(draws, "source/fits/b07.04.samples_flat_priors.rds")
saveRDS(pp_draws, "source/fits/b07.04.flat_priors_post_pred_samples.rds")
...
# Load the draws saved in the previous block
draws <- readRDS("source/fits/b07.04.samples.rds")
...
draws |>
 ...
 # mutate(estimate = exp(estimate)) |>
  ...
  mutate(.variable = case_when(
   ...
   .variable == "b[2]" ~ "b_Age",
   .variable == "b[3]" ~ "b_EI Status",
   .variable == "b[4]" ~ "b_Adapt",
   .variable == "b[5]" ~ "b_Collab",
   .variable == "lam[1,1]" ~ "Loading for m1",
   .variable == "lam[2,1]" ~ "Loading for m2",
   .variable == "lam[3,1]" ~ "Loading for m3",
   .variable == "lam[4,2]" ~ "Loading for m4",
