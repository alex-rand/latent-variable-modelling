...
Now we can just implement this reparameterization when we go to create our event times. We'll also imagine censoring times to be drawn from an unconditional exponential distribution, which creates non-informative censoring. We'll generate an event time and a censoring time for each person, and a person's status will be the lower of those two times. 
...
fake_dat <- fake_dat |>
  ...
  #  lambda_prime = lambda_wiki / exp((beta_f1*f1 + beta_f2*f2 + beta_age*age + beta_ei*ei_status) / rho),
    lambda_prime = lambda_wiki / exp((beta_f1*f1) / rho),
    time_event = rweibull(N, shape=rho, scale=lambda_prime),
    time_censor = rexp(N, censor_rate),
...
test <- survreg(Surv(time, status) ~ f1, dat = fake_dat, 
dist='weibull')
