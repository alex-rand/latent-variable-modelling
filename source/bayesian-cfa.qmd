...
### Report the scale reliabilities?
...
- We can specify the intercept on the raw 'days-to-employment' scale because that helps keep things interpretable. Based on previous studies I've seen at my work, an average time-to-employment of 30 days seems reasonable, and an average higher than 60 days seems very unlikely. 
- The beta coefficients are going to be log acceleration factors. So an exponentiated beta coefficient multiplicatively scales the time to event. For any of the covariates, an acceleration factor of a factor of 5 or more in either direction seems pretty unlikely

We can start with the intercept
...
# Declare the formula
bf <- formula(time | cens(censored) ~ 0 + Intercept + adapt + collab + age + ei_status, family = weibull())
...
priors <- c(
    ...
    prior(normal(0, .5), class = "b", coef = "adapt"),
    prior(normal(0, .5), class = "b", coef = "age"),
    prior(normal(0, .5), class = "b", coef = "collab"),
    prior(normal(0, .5), class = "b", coef = "ei_status")
  )
