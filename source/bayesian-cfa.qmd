...
In all cases the model is __pretty close__ to the true parameter values, shown by the dashed lines. It's a bit worrying that the standard errors are so small they fail to capture the true values, but this illustrates the general proof of concept: we can do a pretty decent CFA in brms. 
...
The model seems to have accurately recovered the true between-factor covariance as well, albeit with some unwholesome weirdness in the tail of its approximated posterior:
...
As mentioned above, we could have instead gone with blavaan for this model. This fits way faster than under the raw Stan approach and recovers the true parameter estimates, but is less flexible overall. And again, crucially for the present analysis, there is no way to do survival analysis with blavaan. 
