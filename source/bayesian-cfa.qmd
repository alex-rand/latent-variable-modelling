...
```
How about cluster-specific KM-curves?
...
plt$data.survplot |> 
  ...
  ggplot() +
  geom_line(aes(x = time, y = surv, group = cluster_id), alpha = .2, colour = "#8A2BE2") +
  theme_bw() +
  scale_x_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0)) +
  labs(
...
```
Before fitting the model we can get a sense of between-cluster variance in parameter estimates by fitting a model to each cluster in isolation and plotting the cluster-specific parameter estimates all together. @GelmanHill2007 call this plot the 'Secret Weapon' _"because it is so easy and powerful but yet is rarely used as a data-analytic tool."_ These non-pooled estimates correspond to a dummy-variable approach for cluster-specific estimates. We'll repeat the two-stage factor score point estimates approach from above as well, since that would be a conventional approach for this situation. We'll focus on the two variables of interest, I.E. the latent variables `adapt` and `collab`:
...
dplt_secret_weapon |> 
  ...
  scale_x_discrete(
    breaks = NULL,  # Removes numeric breaks
    labels = NULL,  # Removes numeric labels
...
The model also does a pretty good job recovering the true values of the standard deviation hyperparameters:
...
```
And the MCMC diagnostics for good measure. All is looking good:
...
#| message: false
#| warning: false
#| fig-cap: "Shrinkage on display: posterior means and credible intervals (purple) are more stable and accurate than unpooled estimates (grey) when compared to true simulated effects (violet rings). The dashed line shows the true simulation mean effect."
...
dplt_secret_weapon |> 
  ...
  geom_errorbar(aes(ymin = posterior_q5, ymax = posterior_q95),
                position = position_dodge(width = 0.5), width = 0.2, colour = "mediumorchid", alpha = .7) +         scale_x_discrete(
    breaks = NULL,  # Removes numeric breaks
    labels = NULL,  # Removes numeric labels
    name = "<---- Smaller Clusters                              Bigger Clusters ---->"  # Custom x-axis title
  ) +
...
fake_dat |>
  ...
  ggplot() +
  geom_histogram(aes(x = time, fill = name, alpha = name), binwidth = 5, colour = "white", position = "identity") +
  scale_fill_manual(values = c("#878787", "cornflowerblue", "firebrick")) +
...
```
The result mirrors what we saw above with the distribution of the mean posterior predictions: the posterior predictive distribution of event times has a wider tail than the observed distribution. The overall density isn't really different between the posterior predictions for the truncated vs untruncated sampling approaches, which reflects the fact that the absurdly long event times in the untruncated approach were only a tiny minority of events. Still, the truncated approach is better because it doesn't allow for these absurdities, making it more useful in a predictive context, for example.
...
ggplot() +
  geom_density(data = dplt |> filter(group == "non-truncated"), aes(x = .value, group = .draw), colour=alpha("cornflowerblue", .5)) + 
