...
saveRDS(fit, "source/fits/b07.02.rds")


```

The model took about 90 minutes to sample and the resulting files are enormous -- on the order of 2.5GB each. Fortunately we are able to selectively load only the draws from variables we're are directly interested in, so we have no problem just working in memory. 
...
# Save the result
saveRDS(test, "source/fits/b07.02.samples.rds")
...
We're interested in the loadings and the between-factor covariance. So let's wrangle the MCMC samples for those variables and plot them to look at the estimated posteriers.

```{r}

fit <- readRDS("source/fits/b07.02.samples.rds")

fit |>

  # Get the raw draws for the factor loadings the beween-factor correlation coefficient
  gather_draws(
    `lam[1,1]`,
    `lam[2,1]`,
    `lam[3,1]`,
