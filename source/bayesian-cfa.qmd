...
model {
  ...
  lam_UNC ~ normal(0, 10); // Prior on loadings
  sd_p ~ cauchy(0, 2.5); // Prior on residual SDs of manifest variables
  cauchy(0, 2.5); // Prior on residual covariance of m1 and m4
  ...
  for (i in 1 : N) {
    ...
    matrix[2, 2] L_sub;  // Cholesky decomposition
    
    mu_sub[1] = mu_UNC[i, 1];
    mu_sub[2] = mu_UNC[i, 4];

    // Set up the raw var-covar matrix for m1 and m4
    sigma_sub[1,1] = square(sd_p[1]);
    sigma_sub[2,2] = square(sd_p[4]);
    sigma_sub[1,2] = theta_m1_m4;
    sigma_sub[2,1] = theta_m1_m4;

    // Get the Cholesky decomp of the raw var-covar matrix
    L_sub = cholesky_decompose(sigma_sub);  // Cholesky decomposition
    
    // Declare the likelihood using the Cholesky decomp of the var-covar matrix. 
    to_vector({X[i, 1], X[i, 4]}) ~ multi_normal_cholesky(mu_sub, L_sub);
  }
