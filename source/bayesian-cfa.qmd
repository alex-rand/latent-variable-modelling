...
factor.score.model |> 
  ...
  ggplot() +
    ...
    geom_linerange(aes(x = reorder(term, -estimate), y = estimate, ymin = conf.low, ymax = conf.high), size = 4, colour =   "cornflowerblue") +
    geom_point(aes(x = reorder(term, -estimate), y = estimate), size = 7, colour = "cornflowerblue") +
    ...
    stat_halfeye(data = cleaned_draws_dplt , aes(x = .variable, y = .value), fill = "mediumorchid", alpha = .8) +
    coord_flip() + 
    labs(
...
rho_lambda_adapt <- log(.3)
rho_lambda_collab <- log(.1)
rho_lambda_age <- log(.2)
rho_lambda_ei <- log(.1)
rho_adapt_collab <- log(.1)
rho_adapt_age <- log(.2)
rho_adapt_ei <- log(.1)
rho_collab_age <- log(.3)
rho_collab_ei <- log(0)
rho_age_ei <- log(.1)
...
Do the simulated data have a realistic-seeming distribution of event times in aggregate?
...
Let's take a look at how the distribution of event times varies from cluster to cluster. We can choose a few select clusters to give a sense of the total range. 

```{r}

# Get the clusters with representative mean times
selected_clusters <- fake_dat |>
...
fake_dat |>
  ...
  ggplot() +
  geom_jitter(aes(x = time, y = cluster_id, colour = as.character(censored)), height = .1, alpha = .6) +
  scale_colour_manual(values = c("cornflowerblue", "firebrick"))
  
```

Or as a concise summary we can plot the median event times from each cluster, in descending order:
...
fake_dat |>
  ...
  ggplot() +
  geom_point(aes(x = median_time, y = cluster_id), size=2, color="mediumorchid", stroke=1) +
  labs(
    x = "Cluster-specific median days employment",
    y = "Clusters",
  ) +
  theme_bw() + 
  theme(
...
And for a fuller sense, the distributions of event times:
...
fake_dat |>
  ...
  ggplot() +
  geom_density_ridges(aes(x = time, y = cluster_id), alpha = 1, rel_min_height = 0.1) +
  xlim(c(0, 100))

```

Before fitting the model we can get a sense of between-cluster variance in parameter estimates by fitting a model to each cluster in isolation and plotting the cluster-specific parameter estimates all together. @GelmanHill2007 call this plot the 'Secret Weapon' _"because it is so easy and powerful but yet is rarely used as a data-analytic tool."_ I use the true simulated values of the latent variables to fit the models here, but could do the two-step approach with the factor scores to make it more realistic? Should probably sort these by cluster size?

```{r}
#| label: secret-weapon
#| message: false
#| warning: false
#| fig-cap: "Andrew Gelman's 'Secret Weapon' plot showing variation between cluster-specific parameter estimates for regression coefficients."
#| fig-height: 30
...
model_list <- map(split_data, function(data) {
  ...
  survreg(surv_obj ~ adapt + collab + age + ei_status, data = data, dist = 'weibull')
})
...
dplt_secret_weapon <- tidy_results |>
  ...
  filter(term %in% c("adapt", "collab", "age", "ei_status")) |>
...
dplt_secret_weapon |> 
  ...
  theme_light() +
#  scale_x_discrete(breaks = seq(from = 0, to = 65, by = 5)) +
...
Let's visualize first the factor loadings and cfa covariance terms, then the substantive regression coefficients: 
...
And how about the estimates of the hyperparameters, so like sd_1 and cor_1?
...
# Get the raw draws
  ...
  mutate(.variable = case_when(
    ...
    .variable == "cor_1" & i == 1 ~ "cor_",
    .variable == "cor_1" & i == 2 ~ "cor_",
    .variable == "cor_1" & i == 3 ~ "cor_",
    .variable == "cor_1" & i == 4 ~ "cor_",
    .variable == "cor_1" & i == 5 ~ "cor_adapt_collab",
    .variable == "cor_1" & i == 6 ~ "cor_adapt_age",
    .variable == "cor_1" & i == 7 ~ "cor_adapt_ei_status",
    .variable == "cor_1" & i == 8 ~ "cor_collab_age",
    .variable == "cor_1" & i == 9 ~ "cor_collab_ei_status",
    .variable == "cor_1" & i == 10 ~ "cor_age_ei__status"
  )) |> 
 ...
 # filter(.variable != "sd_intercept") |> 
  ...
  mutate(true_loading = case_when(
    ...
    .variable == "cor_adapt_collab" ~ exp(.1),
    .variable == "cor_adapt_age" ~ exp(.2),
    .variable == "cor_adapt_ei_status" ~ exp(.1),
    .variable == "cor_collab_age" ~ exp(.3),
    .variable == "cor_collab_ei_status" ~ exp(0),
    .variable ==  "cor_age_ei__status" ~ exp(.1)
  )) |> 
...
```
Cluster-specific comparisons to demonstrate shrinkage? For this we can augment our secret weapon plot from before to compare the no-pooling parameter estimates and the partially pooled estimates from our Stan model:
...
dplt_secret_weapon |> 
  ...
  left_join(
    ...
    fake_dat |> 
      ...
      select(cluster_id, starts_with("beta")) |> 
  ...
  left_join(
    ...
    ranef_summary |> view()
      ...
      # Create the cluster_id variable
      mutate(
  ...
  ggplot(aes(x = factor(cluster_id), y = estimate)) +
  geom_hline(aes(yintercept = true_mean_effect), linetype = 2) +
  geom_point(aes(y = true_cluster_effect), position = position_dodge(width = 0.5), size = 3, colour = "green3", shape = 1, stroke = 1) +
  geom_point(position = position_dodge(width = 0.5), size = 3) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high),
                position = position_dodge(width = 0.5), width = 0.2) +
  geom_point(aes(y = posterior_mean), position = position_dodge(width = 0.5), size = 3, colour = "mediumorchid", alpha = .7) +
  geom_errorbar(aes(ymin = posterior_q5, ymax = posterior_q95),
                position = position_dodge(width = 0.5), width = 0.2, colour = "mediumorchid", alpha = .7) +
  facet_wrap(~ term, scales = c("free"), ncol = 1) +
  theme_light() +
  scale_x_discrete(breaks = seq(from = 0, to = 65, by = 5)) +
  theme(legend.position = "none",
