...
# exp(-0.0121805)
# 
# convert_to_scale <- function(mu, alpha) {
#   scale <- mu / gamma(1 + 1 / alpha)
#   return(scale)
# }
# 
# convert_to_scale(1.01, 0.86)

# Declare the formula
bf <- formula(time | cens(censored) ~ 0 + Intercept + adapt + collab + age + ei_status)


# See which priors are available
get_prior(
  formula = bf,
  data = fake_dat,
  family = weibull()
...
# Figure out what brms has named everything
get_variables(prior.fit)
...
fake_dat |>
  ...
  add_predicted_draws(test.brms, ndraws = 100) |>

  ggplot() + 
  geom_density(aes(x = .prediction, group = .draw), alpha = 0, colour = "mediumorchid") +
  scale_y_continuous(expand=c(0, 0)) +
  xlim(c(0, 365)) +
  theme_minimal()
?scale_y_continuous
...
These look like densities of event times to me. 
