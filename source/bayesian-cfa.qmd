...
fake_dat |>
  ...
  ggplot() +
  geom_point(aes(x = median_time, y = cluster_id), size=3, color="mediumorchid", stroke=1.5) +
  labs(
...
#  geom_point(aes(y = true_cluster_effect), position = position_dodge(width = 0.5), size = 3, colour = "green3", shape = 1, stroke = 2) +
...
saveRDS(ranef_draws_summary, "fits/b07.05.ranef.samples.summary.rds")
...
ranef_draws_summary <- ranef_draws |> summary()
  ...
  left_join(
    ...
    ranef_summary |> 
      ...
      mutate(
        ...
        cluster_id = as.integer(str_extract(variable, "(?<=\\[)\\d+(?=\\])")),
