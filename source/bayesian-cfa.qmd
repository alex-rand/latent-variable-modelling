...
We can plot the event times to make sure they seem realistic:
...
test <- survreg(Surv(time, status) ~ adapt + collab + age + ei_status, data=fake_dat, dist='weibull')
...
Now for the beta coefficients. We can think of ourselves as settings priors as on log acceleration factors, so the exponentiated draws from the prior should reflect our uncertainty about how a one-unit change in that covariate might be associated with a faster or slower time-to-employment, keeping all other measured covariates equal. For any of the covariates, an acceleration factor of 5 or more in either direction (I.E. 5 or 0.2) seems pretty unlikely. As with the intercept, we fiddle with the code until we get a distribution that has the properties we want. much trial and error it seems like a gaussian with `mean = 0, sd = 1` captures this 'factor of 5' intuition, with the a factor of 5 or greater being associated with a probability of about 5% in either direction. 
...
#| message: false
#| warning: false
#| fig-cap: "Densities for 400 draws from the prior predictive distribution of event times."
#| fig-alt: "Densities for 400 draws from the prior predictive distribution of event times."
...
priors <- c(
    ...
    prior(constant(10), class = "shape"), # Constrain the shape parameter, or else it thinks event times can go on basically forever.
    prior(normal(0, 1), class = "b", coef = "adapt"),
    prior(normal(0, 1), class = "b", coef = "age"),
    prior(normal(0, 1), class = "b", coef = "collab"),
    prior(normal(0, 1), class = "b", coef = "ei_status")
  )
...
fake_dat |>
  ...
  add_predicted_draws(fit.prior, ndraws = 400) |>
...
```
 ...
 We can also check the distribution of certain statistics under the prior predictive distribution, like the mean time to event. This is something @Gelman2020 advocate for in their Bayesian Workflow paper. This reveals an area where our model seems a bit dumb: while the mode of the means is well within our desired range of values, there is a very very long tail. This is because the Weibull distribution preserves a long tail of event times,and we haven't told it that event times like 400,000 days are strictly out of the question. And even though the model assigns very tiny probability to these types of numbers, they are still there, non-zero, pulling up the mean of the prior predictive distribution. We can address this when we go to code the raw Stan model by specifying better boundaries on priors and the outcome itself. Or we could change the response distribution entirely, perhaps using a 1-inflated beta distribution to model the proportion of a year until a person found a job, topcoding anybody who took more than a year as 1. But for now let's just stick with the plan. 

```{r}
#| label: prior-pred-1-means
#| message: false
#| warning: false
#| fig-cap: "Mean event times of 400 draws from the prior predictive distribution of event times." 
#| fig-alt: "Mean event times of 400 draws from the prior predictive distribution of event times."
